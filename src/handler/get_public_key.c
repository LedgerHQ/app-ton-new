...
int get_public_key_helper(uint8_t flags, buffer_t *cdata, uint8_t *bip32_path_len, uint32_t bip32_path[MAX_BIP32_PATH], pubkey_ctx_t *pk_info) {
    ...
    if (crypto_derive_public_key(bip32_path,
                                 *bip32_path_len,
                                 pk_info->raw_public_key) < 0) {
...
int handler_get_public_key(uint8_t flags, buffer_t *cdata, bool display) {
    ...
    int ret = get_public_key_helper(flags, cdata, &G_context.bip32_path_len, G_context.bip32_path, &G_context.pk_info);
    if (ret != 0) {
